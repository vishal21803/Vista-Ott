...
$rsMovie = mysqli_query($con, "SELECT * FROM content_info WHERE cname LIKE '%$search%' AND ctype='Series' ");
...
while($row=mysqli_fetch_array($rsMovie))
      ...
      $rating=$row["crating"];
          ...
          echo("</td>");
           echo("<td>");
          echo("<a href='editContentFormSeries.php?cid=$id'>Edit</a>");
          echo("</td>");

            echo("</td>");
           echo("<td>");
          echo("<a href='deleteConfirm.php?content=s&confirm=1&cid=$id'>Delete</a>");
          echo("</td>");
